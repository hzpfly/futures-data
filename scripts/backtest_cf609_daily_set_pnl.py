"""
CF609 Triple Screen 级联回测 — 日线→小时→15分钟 P&L 统计

Periods: 日线 → 小时线 → 15分钟 (中间交易组合)

用法:
    python scripts/backtest_cf609_daily_set_pnl.py
...
def run_backtest(klines_map):
    """
    时序遍历三重滤网 日线→小时→15分钟 级联，管理持仓和 P&L。
    ...
    # ── Screen 1: 探测日线有效数据范围 ──
    print("  计算日线 MACD/EMA 确定有效回测范围...")
    _, _, dif_d, dea_d, hist_d = calc_macd(kd, fast=12, slow=26, signal=9)
    ema13_d = calc_ema(kd, span=13)
    # MACD warmup 33 + EMA lookback 10 + margin 5
    min_warmup = 33 + 10 + 5
    hist_dropna = hist_d.dropna()
    ema_dropna  = ema13_d.dropna()
    if len(hist_dropna) == 0 or len(ema_dropna) == 0:
        print("  ⚠️ 日线数据不足，无法回测")
        return
    first_hist_idx = hist_dropna.index[0]
    first_ema_idx  = ema_dropna.index[0]
    first_valid_d = max(first_hist_idx, first_ema_idx, min_warmup)
    if first_valid_d < 0 or first_valid_d >= n_d:
        print("  ⚠️ 日线有效范围为空，无法回测")
        return
    n_valid_days = n_d - first_valid_d
    print(f"  日线有效范围: bar {first_valid_d}..{n_d-1}  ({n_valid_days} 天)")

    # ── 对齐 15min 数据范围 ──
    # 日线和小时线可能有多年数据，15min 通常只有几个月
    ...
    # 找到第一个覆盖 15min 数据的日线
    first_d_with_k15 = first_valid_d
    while first_d_with_k15 < n_d:
        next_idx = first_d_with_k15 + 1
        t_day_end = (kd.iloc[next_idx]["datetime"] if next_idx < n_d
                     else kd.iloc[first_d_with_k15]["datetime"] + 86400 * 1e9)
        if t_day_end > k15_earliest_ns:
            break
        first_d_with_k15 += 1

    if first_d_with_k15 >= n_d:
        print("  ⚠️ 日线数据与 15min 数据无重叠，无法回测")
        return

    loop_start_d = max(first_valid_d, first_d_with_k15)
    d_date = datetime.fromtimestamp(kd.iloc[loop_start_d]["datetime"] / 1e9).strftime("%Y-%m-%d %H:%M")
    k15_date = datetime.fromtimestamp(k15_earliest_ns / 1e9).strftime("%Y-%m-%d %H:%M")
    print(f"  15min 最早: {k15_date}, 首个重叠日: bar {loop_start_d} ({d_date})")
    print()

    # ── Screen 1 缓存 (默认参数适配日线) ──
    s1_cache = {}
    for i in range(first_valid_d, n_d):
        s1_cache[i] = determine_screen1_trend(kd.iloc[:i+1])

    print(f"开始级联遍历 {loop_start_d}..{n_d-1} 天...")
    total_15m_bars = 0
    ...
    for i in range(loop_start_d, n_d):
        ...
        t_d_start = kd.iloc[i]["datetime"]
        t_d_end   = kd.iloc[i+1]["datetime"] if i+1 < n_d else t_d_start + 86400 * 1e9

        # 小时 bar 在这一天内
        mask_1h = (k1h["datetime"] >= t_d_start) & (k1h["datetime"] < t_d_end)
        idx_1h_list = k1h.index[mask_1h].tolist()

        for j_pos in idx_1h_list:
            s2 = determine_screen2_signal(s1["trend"], k1h.iloc[:j_pos+1])
            ...
            t_1h_start = k1h.iloc[j_pos]["datetime"]
            t_1h_end   = k1h.iloc[j_pos+1]["datetime"] if j_pos+1 < n_1h else t_1h_start + 3600 * 1e9

            # 15min bars in this hour
            mask_15m = (k15["datetime"] >= t_1h_start) & (k15["datetime"] < t_1h_end)
            idx_15m_list = k15.index[mask_15m].tolist()

            for k_pos in idx_15m_list:
                total_15m_bars += 1
                slice_15m = k15.iloc[:k_pos+1]
                ...
                if position is not None and position.status == "open":
                    ...
                    s2_now = determine_screen2_signal(s1_now["trend"], k1h.iloc[:j_pos+1])
                    opp_div = (
                    ...
                    # 4. Trailing stop update (15min level)
...
def _print_results(stats, s1_b, s1_r, s1_n, s2_buy, s2_sell, s2_no, s3_ev, total_bars):
    print(f"\n{'='*72}")
    print(f"  CF609 Triple Screen 级联回测 — 日→时→15min P&L 报告")
    print(f"{'='*72}")

    # ── 级别统计 ──
    print(f"\n── Screen 1 (日线) 趋势分布 ──")
    total_d = s1_b + s1_r + s1_n
    print(f"  看涨 {s1_b:5d}  ({100*s1_b/max(1,total_d):5.1f}%)")
    print(f"  看跌 {s1_r:5d}  ({100*s1_r/max(1,total_d):5.1f}%)")
    print(f"  中性 {s1_n:5d}  ({100*s1_n/max(1,total_d):5.1f}%)")
