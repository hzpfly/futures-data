...
```
加权总分 = EIS_score × 0.5 + TS_score × 0.5

EIS_score = Σ 各周期 EIS 颜色得分 (GREEN=+1, RED=-1, BLUE=0)
TS_score  = S1 趋势 ±1 + S2 回调 ±1 + S3 入场 ±2
```
...
| 加权总分 | 裁决 | 通知? | 含义 |
|----------|------|:-----:|------|
| >= 2.0 | ✅ 强烈可信 — EIS+TS 一致确认 | 🔔 **通知** | 主力仓位 |
| >= 1.0 | ✅ 可信 — 信号方向明确 | 🔔 **通知** | 正常仓位 |
| >= 0.3 | ⚠️ 谨慎 — 信号偏弱 | 🔔 **通知** | 建议半仓 |
| -0.3 ~ 0.3 | ⚠️ 观望 — EIS与TS矛盾 | 🔇 **静默** | 不交易，不显示通知 |
| <= -1.0 | ❌ 做空可信 | 🔔 **通知** | 正常空仓 |
| <= -2.0 | ❌ 强烈做空可信 | 🔔 **通知** | 主力空仓 |

**关键**: 当 EIS 与 Triple Screen 方向矛盾导致加权总分落在 [-0.3, 0.3] 区间时，监控**不弹桌面通知**，终端仅打印一行静默日志。这是信号过滤器，避免弱势/矛盾信号频繁打扰。

**风险提示**: 多周期 EIS 冲突、EIS 与 TS 方向致命冲突时自动告警（仅可信信号触发通知时告警）。
...
### Screen 1 趋势判定
- MACD 柱斜率：最近 2 根 bar-to-bar 差值 > 5%×平均绝对值 → 上升/下降 (Elder: "each new bar is higher than the previous")
...
**信号过滤**: 所有 Triple Screen 信号变化都会先经过 EIS 交叉验证打分。只有当 |加权总分| ≥ 0.3（非"观望"区间）时，才触发桌面通知。得分在 [-0.3, 0.3] 的信号被静默处理，终端仅打印一行日志。
...
5. **EIS 交叉验证 + 信号门控** — 信号触发时自动多周期 EIS 检查，方向一致才高可信；加权总分在 [-0.3, 0.3] 区间时静默处理，不弹通知
